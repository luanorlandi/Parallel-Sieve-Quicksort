...
#pragma omp parallel private(tid, i) num_threads(nthreads)
	{
		...
		for (i = 0; i < nthreads; i++) {
			samples[i + tid * nthreads] =
			    vetor[start[tid] + i * (size[tid] / nthreads)];
		}
	}
	...
	for (i = 1; i < nthreads; i++) {
		if (MPI_Send
		    (pivots, nthreads - 1, MPI_INT, i, TAG,
		     MPI_COMM_WORLD) != MPI_SUCCESS) {
			printf("Houve um erro no envio de uma mensagem.");
			MPI_Finalize();
			exit(1);
		}
		if (MPI_Send(&start[i], 1, MPI_INT, i, TAG, MPI_COMM_WORLD) !=
		    MPI_SUCCESS) {
			printf("Houve um erro no envio de uma mensagem.");
			MPI_Finalize();
			exit(1);
		}
		if (MPI_Send(&size[i], 1, MPI_INT, i, TAG, MPI_COMM_WORLD) !=
		    MPI_SUCCESS) {
			printf("Houve um erro no envio de uma mensagem.");
			MPI_Finalize();
			exit(1);
...
void slave(int *vetor, int tamanho, int nthreads)
{
	...
	pivots = (int *)malloc((nthreads - 1) * sizeof(int));
	MPI_Recv(pivots, nthreads - 1, MPI_INT, 0, TAG, MPI_COMM_WORLD,
		 MPI_STATUS_IGNORE);
